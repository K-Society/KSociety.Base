...
namespace KSociety.Base.Infra.Shared.Class
{
        ...
        where TClassMap : ClassMap<TEntity>
    {
        ...
        {
            Logger.LogTrace("RepositoryBase ImportCsvAsync: {0} {1}.{2}", fileName, GetType().FullName,
            ...
            try
            {
                ...
                await foreach (var entity in ReadCsv<TEntity>.ImportAsync(LoggerFactory, fileName, cancellationToken)
        ...
        {
            Logger.LogTrace("RepositoryBase ImportCsvAsync: {0}.{1}", GetType().FullName,
                System.Reflection.MethodBase.GetCurrentMethod()?.Name);
            try
            {
                ...
                await foreach (var entity in ReadCsv<TEntity>.ImportAsync(LoggerFactory, byteArray, cancellationToken)
                                   .ConfigureAwait(false).WithCancellation(cancellationToken).ConfigureAwait(false))
                {
