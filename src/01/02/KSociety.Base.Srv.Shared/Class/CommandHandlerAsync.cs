...
namespace KSociety.Base.Srv.Shared.Class
{
    ...
    public class CommandHandlerAsync : ICommandHandlerAsync
    {
            ...
            where TResponse : IResponse, new()
        {
            ...
            {
                logger.LogError("ExecuteAsync: {0} - Request: {1} Response: {2} not registered!", type.FullName,
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async ValueTask ExecuteAsync(ILoggerFactory loggerFactory, IComponentContext componentContext,
        ...
        private static async ValueTask ExecutingAsync(ILogger logger, IComponentContext componentContext,
            string serviceName, CancellationToken cancellationToken = default)
        {
            try
            {
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public async ValueTask ExecuteAsync<TRequest>(ILoggerFactory loggerFactory, IComponentContext componentContext,
        ...
        private static async ValueTask ExecutingAsync<TRequest>(ILogger logger, IComponentContext componentContext,
            TRequest request, CancellationToken cancellationToken = default)
            where TRequest : IRequest, new()
        {
            try
            {
                ...
                if (!componentContext.IsRegistered(type))
                {
                    logger.LogError("ExecuteAsync: {0} - Request: {1} not registered!", type.FullName,
            ...
            where TResponse : class, IResponse, new()
        {
            try
            {
                ...
                if (!componentContext.IsRegistered(type))
                {
                    logger.LogError("ExecuteAsync: {0} - Response: {1} not registered!", type.FullName,
